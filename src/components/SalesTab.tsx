  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-base font-bold text-zinc-100">Modul Penjualan & Piutang</h2>
          <p className="text-xs text-zinc-500 font-medium">Buat invoice penjualan, kelola piutang, dan proses pelunasan secara real-time</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white border border-white/[0.06] rounded-xl shadow-sm overflow-hidden flex flex-col">
          <div className="px-5 py-4 border-b border-white/[0.04] bg-white/[0.03]">
            <h3 className="text-xs font-bold text-zinc-500 uppercase tracking-wide">Daftar Sales Invoice</h3>
          </div>
          ...
          <div className="overflow-x-auto flex-1">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-white/[0.04] text-xs text-zinc-300 font-semibold">
                {invoices.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="text-center py-8 text-zinc-500 font-medium">Belum ada invoice penjualan.</td>
                  </tr>
                ) : (
                  invoices.map((inv) => (
                    <tr 
                    ...
                    >
                      ...
                      <td className="px-5 py-4 text-zinc-200">{getCustomerName(inv.customerId)}</td>
                      ...
                      <td className="px-5 py-4 text-center space-x-2" onClick={(e) => e.stopPropagation()}>
                        ...
                        {inv.status === 'Unpaid' && (
                          <button
                            ...
                            className="px-2 py-1 text-[10px] font-bold text-emerald-700 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 rounded-md shadow-sm transition-all duration-200"
                          >
                            Pelunasan
                          </button>
                        )}
        ...
        <div className="bg-white border border-white/[0.06] rounded-xl shadow-sm p-6 flex flex-col space-y-6">
          {selectedInvoice ? (
            <>
              ...
              <div className="border border-white/[0.06] rounded-lg p-5 space-y-4 shadow-sm bg-white/[0.02]/20">
                <div className="flex items-center justify-between border-b border-white/[0.04] pb-3">
                  <div>
                    <h4 className="text-xs font-bold text-zinc-200 uppercase tracking-wide">Faktur Penjualan</h4>
                ...
                <div className="text-[11px] grid grid-cols-2 gap-y-1.5 text-zinc-400 font-semibold">
                  <span>Customer:</span>
                  <span className="text-zinc-100 font-bold text-right">{getCustomerName(selectedInvoice.customerId)}</span>
                  <span>Tanggal:</span>
            ...
            <div className="flex flex-col items-center justify-center h-full text-center py-16 space-y-3">
              ...
              <p className="text-xs text-zinc-500 font-medium">Pilih invoice untuk melihat struk visual dan audit jurnal ledger akuntansi otomatis.</p>
            </div>
          )}
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl border border-white/[0.06] shadow-2xl w-full max-w-3xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="px-6 py-4 border-b border-white/[0.04] flex items-center justify-between bg-white/[0.02]">
              <h3 className="text-sm font-bold text-zinc-100 uppercase tracking-wide">Buat Sales Invoice Baru</h3>
            ...
            <form onSubmit={handlePostInvoice} className="p-6 space-y-4 overflow-y-auto flex-1">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                  <label className="block text-[10px] font-bold text-zinc-500 uppercase mb-1">Customer *</label>
                  <select
                    id="invoice-customer"
                    ...
                    return (
                      <div key={index} className="grid grid-cols-12 gap-3 items-center border border-white/[0.04] p-3 rounded-lg bg-white/[0.01]">
                        ...
                        <div className="col-span-3">
                          <label className="block text-[9px] font-bold text-zinc-500 uppercase mb-0.5">Harga Satuan (Rp)</label>
                          <input
                            type="number"
      ...
      {isPayOpen && selectedInvoice && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl border border-white/[0.06] shadow-2xl w-full max-w-md overflow-hidden flex flex-col">
            <div className="px-6 py-4 border-b border-white/[0.04] flex items-center justify-between bg-white/[0.02]">
              <h3 className="text-sm font-bold text-zinc-100 uppercase tracking-wide">Penerimaan Pembayaran Piutang</h3>
            ...
            <form onSubmit={handlePostPayment} className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-[10px] font-bold text-zinc-500 uppercase mb-1">Setor ke Akun (Kas/Bank) *</label>
                <select
                  className="w-full text-xs px-3 py-2 border border-white/[0.06] rounded-lg focus:outline-none focus:ring-1 focus:ring-slate-800 bg-white"
                  value={payBankCoa}
              ...
              <div>
                <label className="block text-[10px] font-bold text-zinc-500 uppercase mb-1">Tanggal Penerimaan</label>
                <DatePicker
                  value={payDate}
              ...
              <div>
                <label className="block text-[10px] font-bold text-zinc-500 uppercase mb-1">Jumlah Pembayaran (Rp) *</label>
                <input
                  type="number"
